...
function VerificationModal({
  ...
  const handleOTPSubmit = async (data) => {
    ...
    console.log(user?.otp == otp,verificationBy, typeof verificationBy,  verificationBy === "verificationByPhone", "otp tru or false");
    ...
    try {
      if (verificationBy === "verificationByEmail") {
        ...
        if (responseMail) {
          ...
          await getUserProfile({ user_id: user?.user_id });
          onHide();
  ...
  return (
    <>
      ...
      >
        ...
        >
          <p className="mb-3">
            ...
            <b>{user?.otp_send_to || user?.email || user?.phone_number}</b>
          </p>
          <form
            className="mb-3"
      ...
      <VerificationDone show={openVerificationDoneModal} onHide={()=>setOpenVerificationDoneModal(false)}/>
    </>
  );
}
